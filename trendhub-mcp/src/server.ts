...
export function createMcpServer(): McpServer {
  const server = new McpServer(
    { name: SERVER_NAME, version: SERVER_VERSION },
    {
      ...
      instructions:
        "TrendHub is an evidence-first, agent-native professional trend intelligence MCP. Use the stable tools for current hotlists, Xiaohongshu evidence, source reliability, cross-platform resonance, lifecycle/velocity/persistence/diffusion/confidence, lead-time benchmarks, Google Trends, future signals, event calendars, topic analysis, and content briefs. Professional Intelligence v2 adds robust anomaly detection, backtested 6/24/48/72h directional forecasts with uncertainty, evidence-only audience/creator signals, media evidence, alerts and executive reports; workspace_manage adds local-only RBAC/watchlists/saved queries/rules/audit. Missing/degraded data is explicit and must never be fabricated; NOT_COLLECTED/UNAVAILABLE/STALE/OFFLINE/AUTH_REQUIRED/RATE_LIMITED are distinct from observed zero/absence. Use get_trending for raw current lists, trend_intelligence for lifecycle, analyze_topic for a research pack, and professional_intelligence for unified decision support. Forecasts are not probabilities. Sensitive demographics are never inferred. Interpretation and final narrative generation are performed by the caller AI.",
    },
  );
