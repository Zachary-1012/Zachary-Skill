...
>
> **v2.0.3 开源模型标题复核**：TrendHub 自托管 Apache-2.0 多语言模型，网页与 Skill/MCP 使用者无需注册或提供密钥，可按需比较公开标题与主题的语义相似度；不会替代证据、研究结论或创作模型。模型调用不按次收费，托管资源仍有成本。见 [开源模型接入说明](docs/open-model-review.md)。
>
