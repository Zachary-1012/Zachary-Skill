...
if (!gateway.includes('url.pathname === "/api/model-review/status"') || !gateway.includes('url.pathname === "/api/model-review/review"')) fail("open-model status/review routes missing");
if (!gateway.includes("reserveReview(req)") || !gateway.includes("requestsReview(body)")) fail("public model requests must be rate bounded across web and MCP");
if (gateway.includes("TYPESAFE_API_KEY") || gateway.includes("api.typesafe.ai")) fail("paid Jev integration must not remain");
if (!viewsE.includes('id="rvModelReview"')) fail("open-model review must be available in the public research view");
