...
function parseTextResult(result, toolName) {
  ...
  try {
    ...
    throw new Error(`${toolName} returned invalid JSON text: ${error.message}`);
  }
}
...
try {
  ...
  if (names.length !== expectedTools) {
    throw new Error(`expected ${expectedTools} tools, got ${names.length}: ${names.join(", ")}`);
  }
  ...
  if (platforms.length !== expectedPlatforms) {
    throw new Error(`expected ${expectedPlatforms} platforms, got ${platforms.length}: ${platformNames.join(", ")}`);
  }
  if (uniquePlatformNames.size !== expectedPlatforms) {
    throw new Error(`expected ${expectedPlatforms} unique platforms, got ${uniquePlatformNames.size}: ${platformNames.join(", ")}`);
  }
