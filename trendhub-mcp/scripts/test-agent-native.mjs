...
for (const key of ["data", "evidence", "source", "timestamp", "reliability", "confidence", "limitations", "trace"]) assert.ok(key in envelope);\nassert.equal(envelope.contractVersion, "trendhub-evidence-contract-v1");
...
for (const uri of ["trendhub://methodology", "trendhub://namespace", "trendhub://contracts/evidence", "trendhub://skill/trendhub"]) assert.ok(resources.resources.some((x) => x.uri === uri), uri);
const templates = await client.listResourceTemplates();
for (const template of ["platform/{platform}/history", "capability/{capability}", "source/{source}"]) assert.ok(templates.resourceTemplates.some((x) => x.uriTemplate.includes(template)), template);
