...
assert.equal(professional.stableBase, "1.7.2");
assert.equal(professional.targetStableVersion, "1.7.3");
assert.equal(professional.expectedToolCount, 21);
assert.equal(professional.candidateVersion, "1.7.3");
assert.equal(pkg.version, "1.7.3");
assert.notEqual(professional.targetStableVersion, "1.4.5");
const dry = buildPromotionPlan("1.7.3", { dryRun: true });
