...
assert.equal(professional.stableBase, "2.0.3");
assert.equal(professional.targetStableVersion, "2.0.4");
assert.equal(professional.expectedToolCount, 21);
assert.equal(professional.candidateVersion, "2.0.4");
assert.equal(pkg.version, "2.0.4");
assert.notEqual(professional.targetStableVersion, "1.4.5");
const dry = buildPromotionPlan("2.0.4", { dryRun: true });
